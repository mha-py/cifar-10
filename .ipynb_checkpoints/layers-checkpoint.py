...
class PositionalEncoding2d(nn.Module):
    ...
    def forward(self, x):
        b, h, w, c = x.shape
        self.rows.emb = self.rows.emb.to(x.device)
        self.cols.emb = self.rows.emb.to(x.device)
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x, mask=None):
        y = self.ln(x)
        x = x + self.dropout(self.mha(y, y, y, mask, bias=False)) ######
        x = self.ff(x)
        return x
...
class MultiAttentionBlock(nn.Module):
    ...
    def forward(self, x, y):
        xr = x
        x = self.ln1(xr)
        xr = xr + self.mha(x, y, y)
        x = self.ln2(xr)
        xr = xr + self.dense2(gelu(self.dense1(x)))
        return xr
